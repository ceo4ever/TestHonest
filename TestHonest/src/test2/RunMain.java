...
/**
 * 원금, 이자율, 대출기간(월) 이 주어질때 매 월 상환금액을 원금과 이자로 구분하여 출력하는 프로그램을 작성해주세요
 * a. 입력 : 대출금, 대출금리(연리), 대출기간(개월수)
 * b. 출력 : 매 월별 상환 원금 / 이자 금액
 * c. 대출은 시작월의 1 일에 실행되는 것으로 가정하며 일단위 계산은 고려하지 않음
 *
 *<< 로직 설명 
 ...
 *
 *<< 호출 설명
 *java test2.RunMain  대출금액 대출이자 대출기간
 * ==> java test2.RunMain 20000000 4.8 12
 */
public class RunMain {
	...
	public static void main(String[] args) {
		...
		if(args.length != 3){
			System.out.println("입력 정보를 확인바랍니다(입력예시:대출금 대출금리(%) 대출기간(개월수) => 20000000 4.8 12");
			System.exit(-1);
		}
	    ...
	    for(int i = 0; i < paramPeriod; i++){
            ...
            //매월 상환 원금
	    	long repayAmt = paramAmt / paramPeriod;
	    	repayAmt     -= repayAmt % 10 ; //일자리에서 절사 
            //매월 상환 이자금액
	    	long intAmt   = doInterest(amtRepayAfter, paramIntRate);
	    	
	        //마지막 회차에 자투리 금액을 원금에 합침
	        if((i+1) == paramPeriod){
	        	repayAmt = repayAmt + (paramAmt - repayAmt * paramPeriod);
	        }
